...
func (ur DriverRepositorySqlite) FindById(userId uint) (*domain.Driver, error) {
	row := ur.Db.QueryRow("SELECT * FROM Users WHERE id = ? AND company_id IS NOT NULL", userId)
	...
	driver := domain.RestoreDriver(int(id), name, _email, passwordHash, int(companyId.(int64)))
	return driver, nil
}
